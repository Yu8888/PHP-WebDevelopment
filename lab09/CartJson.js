        ...
        if ((this.readyState == 4) && (this.status == 200)) {
		    ...
		    if (this.responseText != "") {
                ...
                if (document.getElementById(myObj.book[id]) == null) {
					...
					row.insertCell(4).innerHTML = `<button class="btn btn-warning" onclick="selectItem('${row.rowIndex}', '${id}')">Select Item</button><br>`;
                } else {
...
function selectItem(row, id) {
	 var cart = document.getElementById("cart");
     cart.rows[row].style.backgroundColor ="pink";
